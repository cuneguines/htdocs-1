<?php    
    ...
    function get_attachments_item_code($itemcode,$depth){
        ...
        $get_file_names = "SELECT t0.ItemCode , t2.trgtPath, t2.FileName, t2.FileExt from oitm t0 LEFT join atc1 t2 on t0.AtcEntry=t2.AbsEntry WHERE t0.Attachment IS NOT NULL AND t0.ItemCode = '$itemcode' AND t2.AbsEntry IS NOT NULL";
        ...
        foreach($file_names_data as $fname){
            array_push($filenames,$fname["trgtPath"]."\\".$fname["FileName"].".".$fname["FileExt"]);
        }
        ...
        foreach($undisplayable_files as $fname){
            ...
            echo $name." <a href='$fname' download='$name' rel='noreferrer noopener'>Download</a><br>";
        };
    }
