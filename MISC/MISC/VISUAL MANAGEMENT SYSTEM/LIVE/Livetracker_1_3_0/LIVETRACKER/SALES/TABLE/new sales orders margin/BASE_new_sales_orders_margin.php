...
<head>
    ...
    <?php include './SQL_new_sales_orders_margin.php'; ?>
    <?php
                ...
                <table id="new_sales_orders_margin" class="filterable sortable colborders">
                    ...
                    <tbody class="white">
                        <?php foreach ($new_sales_orders_margin_data as $sales_order) : ?>
                            ...
                            <?php $cell_color = $sales_order["Margin"] < 0.4 ? 'light_red' : '';?>
                            <tr class='smedium btext' customer='<?= $customer ?>' project='<?= $project ?>' engineer='<?= $engineer ?>' sales_person='<?= $sales_person ?>'>
                                <td><button class = "brred rounded btext medium" onclick = "location.href='./BASE_new_sales_orders_margin_items.php?so=<?=$sales_order['Sales Order']?>'"><?= $sales_order["Sales Order"]?><button></td>
                                <td class='lefttext'><?= $sales_order['Customer'] ?></td>
                                <td class='lefttext'><?= $sales_order["Project"] ?></td>
                                <td><?= $sales_order["Created"] ?></td>
