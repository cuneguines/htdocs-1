    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.multiselector_checkbox').click(function() {
                ...
                document.getElementById("select_status").style.visibility = 'hidden';
                document.getElementById("select_new_stat").style.display = "block";

                //On change of filter shows the visible rows containts selected value
                ...
                //New Filter for the MultiSelector
                rowss = $("table.searchable tbody tr:visible");
                console.log(rowss);
                $("#select_new_stat").on("change", function filter() {
                    var data = this.value;
                    console.log(data);
                    //var rows = $("#production_exceptions").find("tr:not('.head')");
                    console.log(rowss);
                    if (data == 'All') {
                        rowss.show();
                    } else {
                        rowss.hide();
                        console.log(rowss.filter(":contains('" + data + "')").show());
                ...
                <table id="production_exceptions" class="sortable filterable searchable">
                    ...
                    <tbody>
                        ...
                        <?php foreach ($production_exceptions_results as $row) : ?>
                            ...
                            <!-- IF PAGE IS ACCESED BY 'po' POST DO NOT PRINT CUSTOMER COLUMN AN INSTEAD PRINT A IN STOCK AND COMMITTED QTY COLUMN (NOT ENOUGH SPACE FOR BOTH) (IF PAGE IS ACCESSED BY MAIN MENU WITH NO 'po' POST DO THE OPPOSITE)-->
                            <tr status='<?= $status ?>' stage='<?= $stage ?>' overdue='<?= $overdue ?>' supplier='<?= $supplier ?>' customer='<?= $customer ?>' engineer='<?= $engineer ?>' class='white btext smedium' <?= $rowcolor ?>>
            ...
            <div id="grouping_pages_footer" class="footer">
                ...
                <div id="filter_container" style="width:75%;margin-left:2%;margin-right:2%">
                    <div id="filters" class="fill red rounded">
                        <div class="filter widers">
                            ...
                            <div class="content">
                                <select id="select_status" class="selector fill medium">
                                    ...
                                    <?php generate_filter_options($production_exceptions_results, "Sub_Con_Status"); ?>
                                </select>
                                <select id="select_new_stat" class="fill medium" style="position:relative;bottom:98%;display:none">
                                    <option value="" selected>All</option>
                                    <?php generate_filter_optionss($production_exceptions_results, "Sub_Con_Status"); ?>
                                </select>
                            </div>
                        </div>
                        <div class="filter widers">
                            ...
                            <div class="content">
                                <select id="select_new_stat" class="fill medium">
                                    <option value="" selected>All</option>
                                    <?php generate_filter_optionss($production_exceptions_results, "Sub_Con_Status"); ?>
                                </select>
                            </div>
                        ...
                        <div class="filter widers" style="float:left;margin-left:5%">
