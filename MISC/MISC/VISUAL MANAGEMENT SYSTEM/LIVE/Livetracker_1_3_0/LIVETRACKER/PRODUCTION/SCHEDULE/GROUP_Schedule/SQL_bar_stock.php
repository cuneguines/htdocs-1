...
SELECT
	...
	LEFT JOIN(select 
            ...
            where t0.Status = 'R' and t0.Status <> 'C'
            and t1.IssuedQty < t1.PlannedQty
            and t3.ItemType <> 'L'
          --- and t4.ItmsGrpNam LIKE '%Angle%' OR t4.ItmsGrpNam LIKE '%Box%' OR t4.ItmsGrpNam LIKE '%Flat%' OR t4.ItmsGrpNam LIKE '%Round%' OR t4.ItmsGrpNam LIKE '%Sch[edual]%' OR t4.ItmsGrpNam LIKE '%Square%' OR t4.ItmsGrpNam LIKE '%Super%' OR t4.ItmsGrpNam LIKE '%U-Beam%'
$clause
            GROUP BY PrORder
	)t16 ON t16.PrOrder = t1.U_IIS_proPrOrder
...
SELECT
   ...
   LEFT JOIN(select 
				...
				and t1.IssuedQty < t1.PlannedQty
				and t3.ItemType <> 'L'
				--and t1.PlannedQty > t3.onhand 

				GROUP BY PrORder
	)t16 ON t16.PrOrder = t0.U_IIS_proPrOrder
   
   WHERE t0.Status not in ('D','L','C')
   and t0.OriginNum is null
            ) t0
			WHERE t0.[BOM ISSUE PR] IS NOT NULL 
