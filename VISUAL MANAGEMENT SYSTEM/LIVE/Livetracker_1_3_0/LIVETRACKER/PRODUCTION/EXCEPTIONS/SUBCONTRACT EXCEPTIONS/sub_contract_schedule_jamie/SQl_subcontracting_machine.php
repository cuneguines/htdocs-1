...
when t0.[Due Date] = CAST(GETDATE() AS DATE) then '2.TD'
WHEN t0.[Due Date] = DATEADD(DAY,+1,CAST(GETDATE() AS DATE)) THEN '3.TW'
when t0.[Due Date] <= dateadd(wk,1,GETDATE()) and t0.[Due Date] > GETDATE() then '4.One Week'
when t0.[Due Date] <= dateadd(wk,2,GETDATE()) and t0.[Due Date] > dateadd(wk,1,GETDATE()) then '5.Two Weeks'
when t0.[Due Date] <= dateadd(wk,3,GETDATE()) and t0.[Due Date] > dateadd(wk,2,GETDATE()) then '6.Three Weeks'
when t0.[Due Date] <= dateadd(wk,4,GETDATE()) and t0.[Due Date] > dateadd(wk,3,GETDATE()) then '7.Four Weeks'
else '8.Older' end) [Due Within]
...
FROM (
    ...
    SELECT  t1.U_IIS_proPrOrder [Process Order],
          ...
          AND t2.PrcrmntMtd = 'B' AND t0.PlannedQty > (t2.ONhand - t2.IsCommited + t0.PlannedQty)  AND t1.CmpltQty < t1.PlannedQty
           and t5.ItmsGrpNam = 'Machined Parts'
