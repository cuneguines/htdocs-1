...
<head>
    ...
    <script>
        ...
        $(".grouping_categorys").click(function(){
        alert("hello");
    ...
    <div id='background'>
        <div id='content'>
            <div id='grouping_buttons_container'>
                <div id='grouping_buttons' class='fw light_grey rounded' style="width:90%; float:left;">
                    <div id='margin'>
                ...
                <div id='grp_2' class='light_grey rounded btext' style='width:7%; height:100%; margin-left:2%; float:left;background-color: #130564;'>
                    <div class="grouping_categorys" style=" height:100%">
                        <button class="stage_due_Today red wtext rounded" style='float:left;width:100%;margin-bottom:5%;height:30%;margin-top:0%'due="Today">Today</button>
                        <button class="stage_due_Tom red wtext rounded" style='float:left;width:100%;height:30%;margin-bottom:5%'due="Tomorrow">Tomorrow</button>
                        <button class="stage_due_day_aft_Tom red wtext rounded" style='float:left;width:100%;height:30%;'due="Day After Tomorrow">Day After</button>
                    </div>
                ...
                <table id="production_exceptions" class="sortable filterable">
                    ...
                    <tbody>
                        ...
                        <?php foreach ($production_exceptions_results as $row) : ?>
                            ...
                            <?php $engineer = str_replace(' ', '', preg_replace("/[^A-Za-z0-9 ]/", '', $row["Engineer"])); ?>
                            <?php $due_within = str_replace(' ', '', preg_replace("/[^A-Za-z0-9 ]/", '', $row["Due Within"])); ?>

                            <!-- IF PAGE IS ACCESED BY 'po' POST DO NOT PRINT CUSTOMER COLUMN AN INSTEAD PRINT A IN STOCK AND COMMITTED QTY COLUMN (NOT ENOUGH SPACE FOR BOTH) (IF PAGE IS ACCESSED BY MAIN MENU WITH NO 'po' POST DO THE OPPOSITE)-->
                            <tr due='<?= $due_within ?>' stage='<?= $stage ?>' overdue='<?= $overdue ?>' supplier='<?= $supplier ?>' customer='<?= $customer ?>' engineer='<?= $engineer ?>' class='white btext smedium' <?= $rowcolor ?>>
                ...
                <div id="filter_container"style="width:75%;margin-left:2%;margin-right:2%">
                    <div id="filters" class="fill red rounded">
                        <div class="filter widers">
                            <div class="text">
                                <button class="fill red medium wtext">Due Within</button>
                            </div>
                            <div class="content">
                                <select id="select_due" class="selector fill medium">
