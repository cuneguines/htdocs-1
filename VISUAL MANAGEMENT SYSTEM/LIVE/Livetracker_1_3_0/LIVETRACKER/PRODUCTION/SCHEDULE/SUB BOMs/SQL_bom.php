...
$tsql ="select  
CASE WHEN t0.[SB COUNT] > 1 THEN 'Y' ELSE 'N' END[SB], 
t0.[SB COUNT]-1 [SB COUNT],
t0.[Sales Order],
t0.[Customer],
t0.[Project],
t0.[Sales Person],
...
left join(
	SELECT U_IIS_proPrOrder [Proces Order], COUNT(DocNum)[SB COUNT] FROM OWOR
	--WHERE U_IIS_proPrOrder IS NOT NULL AND CreateDate > '01/01/2020'
	GROUP BY U_IIS_proPrOrder
)t12 on t12.[Proces Order]=t4.U_IIS_proPrOrder
...
left join(
	...
	--WHERE U_IIS_proPrOrder IS NOT NULL AND CreateDate > '01/01/2020'
	GROUP BY U_IIS_proPrOrder
)t12 on t12.[Proces Order]=t0.U_IIS_proPrOrder
