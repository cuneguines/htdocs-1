...
FROM (
    ...
    SELECT  t1.U_IIS_proPrOrder [Process Order],
            ...
            t2.ItemName,
            t2.PrcrmntMtd, 
			 ( case when t2.PrcrmntMtd = 'B' AND t0.PlannedQty > (t2.ONhand - t2.IsCommited + t0.PlannedQty)  AND t1.CmpltQty < t1.PlannedQty then 'on hand'end) [ON_HAND],
    ...
    SELECT  NULL [Process Order], 
            ...
            t2.ItemName,
            t2.PrcrmntMtd,
			(case when t2.PrcrmntMtd = 'B' AND t0.OpenQty > (t2.ONhand - t2.IsCommited + t0.OpenQty) then 'on hand' end)[ON_HAND],
