...
console.log(data[0]);
        ...
        $.ajax({
            ...
            type: "POST",
            url: "SQL_subcontract_schedule_test",
            data: {
                'item': test
...
console.log(data[0]);
                ...
                if (data[0].length==0)
                {
                    $('.brred').css({'border-color': ''}); 
                }
                else
                {
                $('.brred').css({'border': '3px solid pink'}); 
                }
        ...
        console.log(y);
        value = 'red';
        $("p.smediums").replaceWith("<p class='smediums'style='font-size:1.7vh;border: 1px solid " + y + ";border-width:7px 7px 0 7px;background-color:" + x + ";'>" + "Promise Date" + "</p>");
        $("h2.fifteenth").replaceWith("<h2 style='border: 1px solid " + y + ";border-width:0 7px 7px 7px;background-color:" + x + ";'class = 'inner fifteenth medium'>" + $(this).attr('promise_week_due') + " " + $(this).attr('weeks_on_floor') + "</h2>");
