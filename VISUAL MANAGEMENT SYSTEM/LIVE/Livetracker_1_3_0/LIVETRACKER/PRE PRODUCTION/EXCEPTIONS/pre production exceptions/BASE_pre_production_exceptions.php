                ...
                <div id='grouping_buttons' class='fw light_grey'>
                    <div id='margin'>
                        <div class="grouping_category">
                            <button class="stage_ok quarter medium light_blue wtext rounded-top"
                                stage="9-submitted-to-planning"><?php echo $pre_production_exceptions_counters[SUBMITTED][PPE_SELF]." (".$pre_production_exceptions_counters[SUBMITTED][OK].")"; ?></button>
                            <button class="stage_name half medium dark_grey wtext" stage="All">Submitted to
                                Planning</button>
                            <button class="stage_not_ok quarter medium red wtext rounded-bottom"
                                stage="9-submitted-to-planning-expired"><?php echo $pre_production_exceptions_counters[SUBMITTED][LATE];?></button>
                        </div>
                        <!--
                            -->
                        <div class="grouping_category">
                            <button class="stage_ok quarter medium light_blue wtext rounded-top"
                                stage="1-drawings-approved"><?php echo $pre_production_exceptions_counters[DRAWINGS_APPROVED][PPE_SELF]." (".$pre_production_exceptions_counters[DRAWINGS_APPROVED][OK].")"; ?></button>
                            <button class="stage_name half medium dark_grey wtext" stage="All">Drawings
                                Approved</button>
                            <button class="stage_not_ok quarter medium red wtext rounded-bottom"
                                stage="1-drawings-approved-expired"><?php echo $pre_production_exceptions_counters[DRAWINGS_APPROVED][LATE];?></button>
                        </div>
                        <!--
                            -->
                        <div class="grouping_category">
                            <button class="stage_ok quarter medium light_blue wtext rounded-top"
                                stage="2-awaiting-customer-approval"><?php echo $pre_production_exceptions_counters[AWAITING_DRAWING_APPROVAL][PPE_SELF]." (".$pre_production_exceptions_counters[AWAITING_DRAWING_APPROVAL][OK].")";?></button>
                            <button class="stage_name half medium dark_grey wtext" stage="All">Awaiting Customer
                                Approval</button>
                            <button class="stage_not_ok quarter medium red wtext rounded-bottom"
                                stage="2-awaiting-customer-approval-expired"><?php echo $pre_production_exceptions_counters[AWAITING_DRAWING_APPROVAL][LATE];?></button>
                        </div>
                        <!--
                            -->
                        <div class="grouping_category">
                            <button class="stage_ok quarter medium light_blue wtext rounded-top"
                                stage="3-revised-drawing-required"><?php echo $pre_production_exceptions_counters[REVISED_DRAWING_REQUIRED][PPE_SELF]." (".$pre_production_exceptions_counters[REVISED_DRAWING_REQUIRED][OK].")"; ?></button>
                            <button class="stage_name half medium dark_grey wtext" stage="All">Revised Drawing
                                Required</button>
                            <button class="stage_not_ok quarter medium red wtext rounded-bottom"
                                stage="3-revised-drawing-required-expired"><?php echo $pre_production_exceptions_counters[REVISED_DRAWING_REQUIRED][LATE];?></button>
                        </div>
                        <!--
                            -->
                        <div class="grouping_category">
                            <button class="stage_ok quarter medium light_blue wtext rounded-top"
                                stage="4-awaiting-sample-approval"><?php echo $pre_production_exceptions_counters[AWAITING_SAMPLE_APPROVAL][PPE_SELF]." (".$pre_production_exceptions_counters[AWAITING_SAMPLE_APPROVAL][OK].")"; ?></button>
                            <button class="stage_name half medium dark_grey wtext" stage="All">Awaiting Sample
                                Approval</button>
                            <button class="stage_not_ok quarter medium red wtext rounded-bottom"
                                stage="4-awaiting-sample-approval-expired"><?php echo $pre_production_exceptions_counters[AWAITING_SAMPLE_APPROVAL][LATE];?></button>
                        </div>
                        <!--
                            -->
                        <div class="grouping_category">
                            <button class="stage_ok quarter medium light_blue wtext rounded-top"
                                stage="5-engineer-drawing"><?php echo $pre_production_exceptions_counters[ENGINEER_DRAWING][PPE_SELF]." (".$pre_production_exceptions_counters[ENGINEER_DRAWING][OK].")"; ?></button>
                            <button class="stage_name half medium dark_grey wtext" stage="All">Engineer Drawing</button>
                            <button class="stage_not_ok quarter medium red wtext rounded-bottom"
                                stage="5-engineer-drawing-expired"><?php echo $pre_production_exceptions_counters[ENGINEER_DRAWING][LATE]; ?></button>
                        </div>
                        <!--
                            -->
                        <div class="grouping_category">
                            <button class="stage_ok quarter medium light_blue wtext rounded-top"
                                stage="6-awaiting-further-information"><?php echo $pre_production_exceptions_counters[AWAITING_FURTHER_INFORMATION][PPE_SELF]." (".$pre_production_exceptions_counters[AWAITING_FURTHER_INFORMATION][OK].")"; ?></button>
                            <button class="stage_name half medium dark_grey wtext" stage="All">Awaiting Further
                                Instruction</button>
                            <button class="stage_not_ok quarter medium red wtext rounded-bottom"
                                stage="6-awaiting-further-information-expired"><?php echo $pre_production_exceptions_counters[AWAITING_FURTHER_INFORMATION][LATE];?></button>
                        </div>
                        <!--
                            -->
                        <div class="grouping_category">
                            <button class="stage_ok quarter medium light_blue wtext rounded-top"
                                stage="8-design-concept"><?php echo $pre_production_exceptions_counters[CONCEPT][PPE_SELF]." (".$pre_production_exceptions_counters[CONCEPT][OK].")"; ?></button>
                            <button class="stage_name half medium dark_grey wtext" stage="All">Design Concept</button>
                            <button class="stage_not_ok quarter medium red wtext rounded-bottom"
                                stage="8-design-concept-expired"><?php echo $pre_production_exceptions_counters[CONCEPT][LATE];?></button>
                        </div>
                        <!--
                ...
                <table id="pre_production_exceptions" class="tablesorter filterable">
                    ...
                    <tbody>
                        ...
                        <tr class='white btext' stage='<?=$stage?>' comments='".$row["Comments"]."' <?=$row_color?>
                            ...
                            <td class="lefttext"><?=$row["U_IIS_proPrOrder"]?></td>
                            <td class="lefttext"><?= htmlspecialchars(substr($row["Project"], 0, 20)) . (strlen($row["Project"]) > 20 ? '...' : '') ?></td>
                            <td class="lefttext"><?= htmlspecialchars(substr($row["Description"], 0, 40)) . (strlen($row["Description"]) > 40 ? '...' : '') ?></td>
                            <td><?=$row["Quantity"]?></td>
                            <!-- <td><?//$row["Est Eng Hrs"]?></td> -->
                            <!-- <td><?//$row["Action Week"]?></td> -->
                            <!--   <td><?//$row["Engineer"]?></td>
                                     <td><?//$row["Est Prod Hrs"]?></td>  -->
                            <td><?=$row["Planned_Hrs"]?></td>
                            <td><?=$row["Promise Date"]?></td>
                            <td><?=$row["Promise Week Due"] >= 52 ? $row["Promise Week Due"]." (".($row["Promise Week Due"]-52).")" : $row["Promise Week Due"]?>
                            </td>
                            <td><?=$row["Engineer"]?></td>
                            <td>
                                <?php
    // Assuming $row["Est LS Start Date"] is in 'MM-DD-YY' format (e.g., '08-27-24')
    $dateString = $row["Est LS Start Date"]; // '08-27-24'
    
    // Create a DateTime object using the specific format
    $date = DateTime::createFromFormat('m-d-y', $dateString);
    
    // Check if the date was created successfully
    if ($date) {
        echo $date->format('d/m/y'); // Format to dd/mm/yy
    } else {
         ''; // Handle invalid date format
    }
    ?>
                            </td>
                            <td><button class='comment_button <?= $row["Comments"] != null ? 'has_comment' : ''?>'
                                    comments='<?= $row["Comments"] == null ? "NO COMMENTS" : $row["Comments"]?>'></button>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <div id="grouping_pages_footer" class="footer">
                ...
                <div id="button_container">
                    <button onclick="export_to_excel('pre_production_exceptions')"
