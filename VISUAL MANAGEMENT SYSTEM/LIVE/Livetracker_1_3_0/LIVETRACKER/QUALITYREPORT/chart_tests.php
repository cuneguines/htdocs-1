...
try {
  ...
  while ($startDate <= $endDate) {
      $monthYear = date("F Y", $startDate);
      $months2023[$monthYear] = 0; // Initialize each month with zero opened issues
  ...
  // Fill the data arrays with the correct counts for each month and area
  foreach ($openedIssuesPerMonthAndArea as &$openedIssuesPerArea) {
      $openedIssuesPerArea = array_values($openedIssuesPerArea); // Convert the associative array to an indexed array
  }

  // Sort the data arrays based on month for each area
  ksort($openedIssuesPerMonthAndArea);
