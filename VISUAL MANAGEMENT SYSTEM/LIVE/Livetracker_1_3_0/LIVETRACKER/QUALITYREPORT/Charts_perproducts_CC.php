...
//die;
    ...
    $areas = array_unique(array_reduce($results, function ($carry, $entry) {
        if ($entry['form_type'] === 'Customer Complaints') {
            $carry[] = $entry['U_Product_Group_Two'];
...
<head>
    ...
    ?>
            // Populate the data rows using the $openedIssuesPerMonth array
    var dataArray = <?php echo json_encode($openedIssuesPerMonth); ?>;
    for (var month in dataArray) {
        var row = [month];
        <?php
        foreach ($areas as $area) {
            echo "row.push(dataArray[month]['$area'] !== undefined ? dataArray[month]['$area'] : 0);\n";
        }
        ?>
        data.addRow(row);
    }
            var options = {
                title: 'Count of Issues per Area per Month',
                vAxis: {title: 'Issue Count'},
                hAxis: {title: 'Month'},
                seriesType: 'bars',
                series: {2: {type: 'line'}} // Add a line series for one area
            };
...
<body>
    <div id="chart_div" style="width: 1800px; height: 1900px;"></div>
</body>
</html>
