<?php
// ... Your SQL query and database connection code ...for the year 2023
...
try {
    ...
    foreach ($results as $entry) {
        ...
        if (($formType === 'Opportunity For Improvement'||  $formType ==='Non Conformance')&& $status==='Closed' && date('Y', strtotime($dateUpdated)) === '2023' && strtotime($dateUpdated) <= strtotime('last day of ' . $currentMonthYear)) {
    ...
    $areas = array_unique(array_reduce($results, function ($carry, $entry) {
        if ($entry['form_type'] === 'Opportunity For Improvement') {
            $carry[] = $entry['nc_area_caused'];
    ...
    foreach ($results_op as $entry) {
        ...
        $status=$entry['Status'];
        // Check if the entry is a Customer complaint in the year 2023 and before or equal to the current month
        if (($formType === 'Opportunity For Improvement'||  $formType ==='Non Conformance')&& $status==='Open' && date('Y', strtotime($dateUpdated)) === '2023' && strtotime($dateUpdated) <= strtotime('last day of ' . $currentMonthYear)) {
            // Extract the month and year from the "Date Updated"
            $monthYear = date("F Y", strtotime($dateUpdated));
    ...
    $areas_new = array_unique(array_reduce($results_op, function ($carry_new, $entry) {
        if ($entry['form_type'] === 'Opportunity For Improvement') {
            $carry_new[] = $entry['nc_area_caused'];
...
function drawCharts() {
    ...
    var options = {
        title: 'Number of Issues/Closed by Area Caused and Month',
        isStacked: true,
        vAxis: { title: 'Month' },
