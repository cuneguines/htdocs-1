...
$openedIssuesPerMonth = array();
    ...
    $areas = array_unique(array_reduce($results, function ($carry, $entry) {
        if ($entry['form_type'] === 'Customer Complaints') {
             if (isset($entry['U_Product_Group_Two'])){ $carry[] =$entry['U_Product_Group_Two'];}
...
<head>
    ...
    #chart-container
    {
        ...
        /* Add CSS styling for the chart container */
        #chart_div {
            width: 70%;
            height: 50%;
            margin: 0 auto;
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            ...
            <?php foreach ($areas as $area) { ?>
                data.addColumn('number', '<?php echo $area; ?>');
            <?php } ?>

            var dataArray = <?php echo json_encode($openedIssuesPerMonth); ?>;
            for (var month in dataArray) {
                var row = [month];
                <?php foreach ($areas as $area) { ?>
                    row.push(dataArray[month]['<?php echo $area; ?>'] !== undefined ? dataArray[month]['<?php echo $area; ?>'] : 0);
                <?php } ?>
                data.addRow(row);
            }

            var options = {
    title: 'Customer complaints per product group per month',
...
<body>
    ...
    <h2 style="background-color:white;text-align:center;box-shadow: 0px -2px 8px 6px #9E9E9E;">Count of Customer Complaints per Product per Month for the year 2023</h2>
